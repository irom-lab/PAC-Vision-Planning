...
def test(params, policy, DepthFilter, device, mu, std, seed):
...
if __name__ == "__main__":
    ...
    mu_dict = torch.load('Weights/mu_'+str(save_file_v)+'_best.pt')
    logvar_dict = torch.load('Weights/logvar_'+str(save_file_v)+'_best.pt')
    ...
    for i in range(env_num_start,env_num_start+num_envs):
        ...
        print("==============")
        cost, collision_flag, collision_counter = test(params, policy, DepthFilter, device, mu, std, i)

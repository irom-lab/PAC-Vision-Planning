...
class Compute_Loss:
    ...
    def compute(self, itr_num, params, mu, std, mu_pr, logvar_pr, reg_include, eps_shared):
    ...
    def thread(params, policy, DepthFilter, device, mu, std, mu_pr, logvar_pr, batch_size, np_seed, torch_seed, reg_include, rd, proc_num):
        ...
        for i in range(batch_size):
            ...
            if reg_include:
                grad_mu_temp, grad_logvar_temp = compute_grad_ES(pac_costs-pac_costs.mean(), epsilon, std, num_fit_frac, grad_method, 0)
            else:
                grad_mu_temp, grad_logvar_temp = compute_grad_ES(policy_eval_costs-policy_eval_costs.mean(), epsilon, std, num_fit_frac, grad_method, 0)
